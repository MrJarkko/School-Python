...
This program is for DECRYPTING
'''
...
# Function to do the decryption of text
def decrypt_string(input_file):
    new_string = ''
...
# Will shift all the characters back for 5 places if the are part of an alphabet
            new_asc_char = asc_char - 5
# Check if the characters did not fall out from alphabet scope, and if thay did, give those new value starting from end of alphabet
            if (    (new_asc_char > 90 and new_asc_char < 97)
                or  (new_asc_char < 65)
            ):
...
file_content = read_file(input('Enter encrypted file name : '))

decrypted_text = decrypt_string(file_content)

write_file(input('Enter new file name where decrypted content will be stored : '), decrypted_text)
